...
if(length(reservoirs) <1){print(paste("No reservoirs included within the", include.watersheds, "watershed(s)"))
  ...
  for(i in 1:length(reservoirs)){
    ...
    tmp <- read_xlsx("/var/obwb-hydro-modelling/input-data/raw/reservoirs/okanagan-reservoirs.xlsx", sheet = reservoirs[i])
    ...
    tmp <- tmp[tmp$Current_Storage_dam3 == 0 | tmp$Storage_Type == "Live" | tmp$Storage_Type == "Potential", ]
    
    ## Convert volume to m3 as required by Raven
    tmp$Current_Storage_m3 <- tmp$Current_Storage_dam3 * 1000
    
    StageStorage <- tmp[ ,c("Stage_m_GSC", "Current_Storage_m3")]
